...
Reading guide: §\ref{sec:background} sets the Skillarum context; §\ref{sec:methods} specifies the pipeline; §\ref{sec:results} reports the live run; §\ref{sec:visualizations} presents the figures; §\ref{sec:catalog} catalogs every skill; §\ref{sec:discussion} states limitations.
